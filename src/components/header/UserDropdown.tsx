...
useEffect(() => {
  async function loadProfile() {
    try {
      const response = await fetch(`${configs.NEO_HOST}${configs.PROFILE}`, {
        credentials: "include",
      });
      ...
      setUsername(payload.data?.name || null);
      setUseremail(payload.data?.email || null);
      setUserid(payload.data?.id || null);
    } catch (err) {
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 h-11 w-11 overflow-hidden rounded-full">
          <img
            src={
              `${configs.NEO_HOST}${configs.PROFILE_PHOTO}/${userid}.jpg`
            }
            alt={username || "User"}
      ...
      >
        ...
        <Link
          href= {configs.NEO_HOST + configs.LOGOUT_PATH}
